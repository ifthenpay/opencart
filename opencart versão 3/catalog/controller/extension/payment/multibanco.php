...
class ControllerExtensionPaymentMultibanco extends Controller
{
	...
	public function index()
	{
		...
		$this->document->addScript('extension/payment/javascript/ifthenpay/checkoutMultibancoPage.js');
	...
	public function updateUserAccount(): void
	{
        ...
        if (!$requestUserToken || $requestUserToken !== $this->config->get('payment_multibanco_updateUserAccountToken')) {
        ...
        try {
            ...
            $backofficeKey = $this->config->get('payment_multibanco_backofficeKey');
			...
			$modelAdmin->editSettingValue(
				'payment_multibanco', 
				'payment_multibanco_userPaymentMethods', 
				serialize($ifthenpayGateway->getPaymentMethods())
			); 
			$modelAdmin->editSettingValue(
				'payment_multibanco', 
				'payment_multibanco_userAccount', 
				serialize($ifthenpayGateway->getAccount())
			);
			$modelAdmin->editSettingValue(
				'payment_multibanco',
				'payment_multibanco_updateUserAccountToken',
				''
			);			
            http_response_code(200);
			$this->model_extension_payment_multibanco->log('', 'User Account updated with success!');
            die('User Account updated with success!');
        } catch (\Throwable $th) {
			$this->model_extension_payment_multibanco->log($th->getMessage(), 'Error Updating User Account');
	...
	public function confirm()
	{
		...
		if ($this->session->data['payment_method']['code'] == 'multibanco') {
			...
			try {
				...
				$this->session->data['ifthenpayPaymentReturn']['paymentLogoUrl'] = $this->config->get('site_url') . 'image/payment/ifthenpay/multibanco.svg';
				...
				$this->model_checkout_order->addOrderHistory(
					...
					$configData['payment_multibanco_order_status_id'],
					$comment,
					true,
					true
				);
				...
				$this->model_extension_payment_multibanco->log($th->getMessage(), 'Error Processing Payment');
	...
	public function changeOrderStatusFromWebservice()
	{
		$this->load->language('extension/payment/multibanco');
		$this->load->model('extension/payment/multibanco');
		$this->load->model('checkout/order');
		$this->model_checkout_order->addOrderHistory(
			...
			$this->config->get('payment_multibanco_order_status_complete_id'),
			$this->language->get('paymentConfirmedSuccess'),
			true,
			true
		);
		$this->model_extension_payment_multibanco->log($this->request->get['order_id'], 'Payshop Order Status Change to paid with success');
	}
	...
	public function callback()
	{
		try {
			...
			$this->model_extension_payment_multibanco->log($this->request->get, 'Callback Processed with Success.');
        } catch (\Throwable $th) {
			$this->model_extension_payment_multibanco->log($this->request->get . '-' . $th->getMessage(), 'Error Processing callback.');
	...
	public function changeFooterScripts(&$route, &$data, &$output) {
		$this->document->addScript('catalog/view/javascript/ifthenpay/checkoutMultibancoPage.js');
	...
	{
		if ($this->session->data['payment_method']['code'] == 'multibanco') {
			$this->session->data['ifthenpayPaymentReturn']['paymentMethodLogo'] = $this->config->get('site_url') . '/image/payment/ifthenpay/multibanco.svg';
