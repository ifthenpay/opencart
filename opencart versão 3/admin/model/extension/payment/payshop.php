...
class ModelExtensionPaymentPayshop extends Model {
	...
	public function getAllPayshopPendingOrders(): array
	{
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order WHERE `payment_code` = 'payshop' AND `order_status_id` =" . $this->config->get('payment_payshop_order_status_id'));
