...
class Mbway extends \Opencart\System\Engine\Controller
{
	...
	{
		if (!$this->config->get('payment_mbway_show_icon_checkout') || !isset($data['header'])) {
			return;
		}

		$data['header'] .= Utils::getPaymentIconCssInjectionScript('mbway');
	}
	...
	{
		(new CallbackService($this->registry))->HandleFromMbway($this->request);
	}
