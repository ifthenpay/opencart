...
class Ifthenpaygateway extends \Opencart\System\Engine\Controller
{
	...
	/**
	 * Injects a js script that load css into the header, this will be called in the checkout page event
	 */
	public function injectIconCss(&$route, &$data)
	{
		...
		if ($this->config->get('payment_ifthenpaygateway_show_icon_checkout') === '1') {
			$data['header'] .= Utils::getPaymentIconCssInjectionScript(strtolower(self::PAYMENTMETHOD));
		}
		...
		if ($this->config->get('payment_ifthenpaygateway_show_icon_checkout') === '2') {
			...
			foreach ($paymentMethods as $key => $value) {
				if (isset($value['is_active']) && $value['is_active'] === '1') {
					...
					$activePaymentMethods[] = $lowerCaseKey === 'mb' ? 'multibanco' : $lowerCaseKey;
				}
			}

			$data['header'] .= Utils::getIfthenpaygatewayIconCssInjectionScript($activePaymentMethods);
		}
	}
	...
	public function email_payment_info(&$route, &$data)
	{
		...
		if (!isset($paymentInfo) || !isset($paymentInfo['payment_method']) || $paymentInfo['payment_method'] != 'ifthenpaygateway') {
			return;
		}
	...
	private function getPaymentDetailsHtml($isForAdmin = false, $isForEmail = false)
	{
		...
		if ($isForEmail) {
			return $this->load->view('extension/ifthenpay/payment/ifthenpaygatewayOrderEmailDetails', $params);
		}
