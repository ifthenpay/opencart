...
class Payshop extends \Opencart\System\Engine\Model
{
	...
	public function getMethods(array $address, float $total = 0.0): array
	{
		...
		$geoZoneId = $this->config->get('payment_payshop_geo_zone_id');
		if ($geoZoneId) {
		...
		$minAmount = $this->config->get('payment_payshop_min_value');
		$maxAmount = $this->config->get('payment_payshop_max_value');
		if (
		...
		$option_data['payshop'] = [
			'code' => 'payshop.' . 'payshop',
			'name' => $this->config->get('payment_payshop_title')
		];
	...
	/**
	 * adds a new payshop record (representing an order) to the database
	 * @param array $data
	 * @return void
	 */
	public function addPayshopRecord(array $data): void
	{
		$this->db->query("INSERT INTO `" . DB_PREFIX . "ifthenpay_payshop` SET " .
	...
	/**
	 * gets all payshop records that have status = pending from database
	 * @return array
	 */
	public function getPayshopRecordsByPendingStatus()
	{
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ifthenpay_payshop` WHERE `status` = 'pending'");
	...
	/**
	 * updates the status of a payshop record
	 * @param int $order_id
	 * @param string $status
	 * @return void
	 */
