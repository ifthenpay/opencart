...
class CallbackService
{
	...
	public function HandleFromPix(Request $request)
	{

		try {

			// set this config values to instance to later validate against callback data
			$this->isCallbackActive = $this->registry->config->get('payment_pix_activate_callback');
			$this->antiPhishingKey = $this->registry->config->get('payment_pix_anti_phishing_key');

			$transactionId = isset($request->get['transaction_id']) ? $request->get['transaction_id'] : '';
			$orderId = isset($request->get['order_id']) ? $request->get['order_id'] : '';

			// check if there is a record in pix table
			$this->registry->load->model('extension/ifthenpay/payment/pix');
			$storedData = $this->registry->model_extension_ifthenpay_payment_pix->getPixRecordByTransactionId($transactionId);
			...
			$this->logger->write('IFTHENPAY - HandleFromPix - ERROR : ' . $th->getMessage());
	...
	{
		$this->registry->load->model('extension/ifthenpay/payment/multibanco');
		$this->registry->load->language('extension/ifthenpay/payment/multibanco');
	...
	{
		$this->registry->load->model('extension/ifthenpay/payment/mbway');
		$this->registry->load->language('extension/ifthenpay/payment/mbway');
	...
	{
		$this->registry->load->model('extension/ifthenpay/payment/payshop');
		$this->registry->load->language('extension/ifthenpay/payment/payshop');
	...
	private function processCallbackForIfthenpayGateway(Request $request, array $storedDataIfthenpaygateway)
	{
		...
		// prepare a comment, since we can say what method was used
		$comment = $this->registry->language->get('comment_paid');

		$methodStr = $request->get['pmt'] ?? null;

		if (!$methodStr) {
			$urlRoute = $request->get['route'] ?? '';
			if (preg_match('/payment\/([^\.|\|]+)/', $urlRoute, $matches)) {
				$methodStr = $matches[1];
			}
		}
		...
		switch ($methodStr) {
			case 'multibanco':
				$method = $this->registry->language->get('str_multibanco');
				break;
			case 'mbway':
				$method = $this->registry->language->get('str_mbway');
				break;
			case 'payshop':
			...
			case 'pix':
				...
				break;
			case 'ccard':
				$method = $this->registry->language->get('str_ccard');
				break;
			case 'google':
				$method = $this->registry->language->get('str_google');
				break;
			case 'apple':
				$method = $this->registry->language->get('str_apple');
				break;
		...
		if ($method) {
			$comment = sprintf($this->registry->language->get('comment_paid_with'), $method);
		}
	...
	private function processCallbackForPix(Request $request, array $storedData)
	{
		$this->registry->load->model('extension/ifthenpay/payment/pix');
		$this->registry->load->language('extension/ifthenpay/payment/pix');
		...
		$this->validateCallbackPix($request->get, $storedData);
	...
	private function validateCallbackPix($callbackData, $storedPaymentData): void
	{
