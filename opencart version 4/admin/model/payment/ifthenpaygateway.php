...
class Ifthenpaygateway extends \Opencart\System\Engine\Model
{
	public function install(): void
	{
		...
		$this->model_setting_event->addEvent([
			'code' => 'payment_ifthenpay_ifthenpaygateway_email',
			'description' => 'To display Ifthenpay Gateway payment information in the order confirmation email.',
			'trigger' => 'catalog/view/mail/order_add/before',
			'action' => 'extension/ifthenpay/payment/ifthenpaygateway.email_payment_info',
			'status' => 1,
			'sort_order' => 2
		]);

		$this->model_setting_event->addEvent([
			'code'        => 'payment_ifthenpay_ifthenpaygateway_icon_injection',
			'description' => 'Injects CSS for Ifthenpaygateway payment method icon in the checkout page.',
			'trigger'     => 'catalog/view/checkout/checkout/before',
			'action'      => 'extension/ifthenpay/payment/ifthenpaygateway.injectIconCss',
			'status'      => 1,
			'sort_order'  => 1
		]);
	}
