...
class Cofidis extends \Opencart\System\Engine\Controller
{
	...
	/**
	 * generate the cofidis payment configuration page
	 * this is the page where the merchant can configure the cofidis payment
	 * @return void
	 */
	public function index(): void
	{
		...
		$data['url_test_callback'] = $this->url->link('extension/ifthenpay/payment/cofidis.ajaxTestCallback', 'user_token=' . $this->session->data['user_token'], true);
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/ifthenpay/payment/cofidis', 'user_token=' . $this->session->data['user_token'])
		];
		$data['save'] = $this->url->link('extension/ifthenpay/payment/cofidis.save', 'user_token=' . $this->session->data['user_token']);
		...
		$data['backoffice_key'] = $this->config->get('payment_cofidis_backoffice_key');

		$data['cofidis_status'] = $this->config->get('payment_cofidis_status');
		$data['cofidis_activate_callback'] = $this->config->get('payment_cofidis_activate_callback');


		// entities/keys related values
		$accounts = $this->config->get('payment_cofidis_accounts');
		$data['cofidis_key'] = $this->config->get('payment_cofidis_key');
		$data['cofidis_keys_options'] = Gateway::accountsToKeyOptions(json_decode($accounts, true));


		$data['cofidis_min_value'] = $this->config->get('payment_cofidis_min_value');
		$data['cofidis_max_value'] = $this->config->get('payment_cofidis_max_value');
		...
		$data['cofidis_geo_zone_id'] = $this->config->get('payment_cofidis_geo_zone_id');
		$this->load->model('localisation/geo_zone');
		$data['geo_zones'] = $this->model_localisation_geo_zone->getGeoZones();

		$data['cofidis_sort_order'] = $this->config->get('payment_cofidis_sort_order');

		// callback info related values
		$data['cofidis_url_callback'] = $this->config->get('payment_cofidis_url_callback');
		$data['cofidis_anti_phishing_key'] = $this->config->get('payment_cofidis_anti_phishing_key');
		...
		$this->response->setOutput($this->load->view('extension/ifthenpay/payment/cofidis', $data));
	}
	...
	private function requestGetCofidisMaxMinAmount(string $cofidisKey): array
	{
		$maxMinAmountArray = ['max' => '', 'min' => ''];

		if ($cofidisKey == '') {
			return $maxMinAmountArray;
		}

		$result = json_decode((new ApiService())->requestCofidisMaxMinAmount($cofidisKey), true);
		if (isset($result['message']) && $result['message'] == 'success') {
			$maxMinAmountArray['max'] = $result['limits']['maxAmount'];
			$maxMinAmountArray['min'] = $result['limits']['minAmount'];
		}
	...
	public function save(): void
	{
		...
		if (empty($savedBackofficeKey)) {
			...
			$inputtedBackofficeKey = $this->request->post['payment_cofidis_backoffice_key'] ?? '';
		...
		if (!isset($this->json['error'])) {
			...
			$this->json['redirect'] = $this->url->link('extension/ifthenpay/payment/cofidis', 'user_token=' . $this->session->data['user_token'], true);
		}
	...
	private function validate($formData): bool
	{
		...
		if ($formData['payment_cofidis_key'] == '') {
		...
		if ($formData['payment_cofidis_min_value'] !== '' && $formData['payment_cofidis_max_value'] !== '' && $formData['payment_cofidis_min_value'] > $formData['payment_cofidis_max_value']) {
	...
	public function install(): void
	{
		if ($this->user->hasPermission('modify', 'extension/payment')) {
			...
			$this->model_extension_ifthenpay_payment_cofidis->install();
		}
	}
	...
	public function uninstall(): void
	{
		if ($this->user->hasPermission('modify', 'extension/payment')) {
			...
			$this->model_extension_ifthenpay_payment_cofidis->uninstall();
		}
	}
	...
	public function ajaxTestCallback(): void
	{
		...
		if (!isset($json)) {
			...
			$callbackUrl = $this->config->get('payment_cofidis_url_callback');
			$antiPhishingKey = $this->config->get('payment_cofidis_anti_phishing_key');
	...
	public function ajaxGetMaxMinAmountLimits(): void
	{
		$key = $this->request->get['key'] ?? '';
		$maxMinAmountArray = $this->requestGetCofidisMaxMinAmount($key);

		$json['success'] = $maxMinAmountArray;
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	}
	...
	/**
	 * clear the configuration of the extension, deleting all the settings from database related to cofidis
	 * @return void
	 */
	public function ajaxClearConfiguration(): void
	{
		...
		$this->model_setting_setting->deleteSetting('payment_cofidis');
	...
	public function ajaxRequestAccount(): void
	{
		...
		$data['payment_method'] = 'Cofidis Pay';
		$data['ecommerce_platform'] = 'Opencart ' . $opencartVersion;
		$data['module_version'] = Utils::getModuleVersion();
		$data['refresh_accounts_url'] = $this->url->link('extension/ifthenpay/payment/cofidis.refreshAccountsCtrl', 'user_token=' . $this->session->data['user_token']);
