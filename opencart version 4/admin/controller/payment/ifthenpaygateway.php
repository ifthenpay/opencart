...
class Ifthenpaygateway extends \Opencart\System\Engine\Controller
{
	...
	public function index(): void
	{
		...
		$data['ifthenpaygateway_show_icon_checkout'] = $this->config->get('payment_ifthenpaygateway_show_icon_checkout');
	...
	private function validate($formData): bool
	{
		...
		if ($formData['payment_ifthenpaygateway_min_value'] !== '' && (!is_numeric($formData['payment_ifthenpaygateway_min_value']) || $formData['payment_ifthenpaygateway_min_value'] < 0)) {
			$this->json['error'] = $this->language->get('error_min_value_format');
			return false;
		}

		if ($formData['payment_ifthenpaygateway_max_value'] !== '' && (!is_numeric($formData['payment_ifthenpaygateway_max_value']) || $formData['payment_ifthenpaygateway_max_value'] < 0)) {
		...
		if ($formData['payment_ifthenpaygateway_deadline'] !== '' && (!is_numeric($formData['payment_ifthenpaygateway_deadline']) || (int)$formData['payment_ifthenpaygateway_deadline'] <= 0)) {
			...
			<div class="method_line row align-items-center mb-3 border-0">
				<div class="col-12 col-sm-auto d-flex align-items-center mb-2 mb-sm-0">
					<div class="form-check form-switch form-switch-lg">
						...
						<input type="checkbox" id="{$paymentMethodGroup['Method']}" name="payment_ifthenpaygateway_methods[{$paymentMethodGroup['Entity']}][is_active]" value="1" class="form-check-input is_active" {$isChecked} {$isDisabled} data-method="{$paymentMethodGroup['Entity']}" onchange="updateDefaultSelect(this)"/>
					</div>
					<label for="{$paymentMethodGroup['Method']}" class="payment_logo_div ml-3">
