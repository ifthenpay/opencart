...
class Pix extends \Opencart\System\Engine\Controller
{
	...
	/**
	 * generate the pix payment configuration page
	 * this is the page where the merchant can configure the pix payment
	 * @return void
	 */
	public function index(): void
	{
		...
		$data['url_clear_configuration'] = $this->url->link('extension/ifthenpay/payment/pix.ajaxClearConfiguration', 'user_token=' . $this->session->data['user_token'], true);
		$data['url_request_account'] = $this->url->link('extension/ifthenpay/payment/pix.ajaxRequestAccount', 'user_token=' . $this->session->data['user_token'], true);
		$data['url_refresh_accounts'] = $this->url->link('extension/ifthenpay/payment/pix.ajaxRefreshAccounts', 'user_token=' . $this->session->data['user_token'], true);
		$data['url_test_callback'] = $this->url->link('extension/ifthenpay/payment/pix.ajaxTestCallback', 'user_token=' . $this->session->data['user_token'], true);
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/ifthenpay/payment/pix', 'user_token=' . $this->session->data['user_token'])
		];
		$data['save'] = $this->url->link('extension/ifthenpay/payment/pix.save', 'user_token=' . $this->session->data['user_token']);
		...
		$data['backoffice_key'] = $this->config->get('payment_pix_backoffice_key');

		$data['pix_status'] = $this->config->get('payment_pix_status');
		$data['pix_activate_callback'] = $this->config->get('payment_pix_activate_callback');

		$data['pix_cancel_cronjob'] = $this->config->get('payment_pix_cancel_cronjob');
		$urlCronCancel = str_replace(HTTP_SERVER, HTTP_CATALOG, $this->url->link('extension/ifthenpay/payment/pix|cronCancelOrder'));
		$data['pix_cancel_cronjob_url'] = 'wget -q -O /dev/null "' . $urlCronCancel . '" --read-timeout=5400';

		// entities/keys related values
		$accounts = $this->config->get('payment_pix_accounts');
		$data['pix_key'] = $this->config->get('payment_pix_key');
		$data['pix_keys_options'] = Gateway::accountsToKeyOptions(json_decode($accounts, true));

		$data['pix_min_value'] = $this->config->get('payment_pix_min_value');
		$data['pix_max_value'] = $this->config->get('payment_pix_max_value');

		// title related values
		$title = $this->config->get('payment_pix_title');
		$data['pix_title'] = $title != '' ? $title : $this->language->get('heading_title');
		$data['pix_show_icon_checkout'] = $this->config->get('payment_pix_show_icon_checkout');



		// order status related values
		$pendingStatus = $this->config->get('payment_pix_pending_status_id');
		$paidStatus = $this->config->get('payment_pix_paid_status_id');
		$canceledStatus = $this->config->get('payment_pix_canceled_status_id');
		$data['pix_pending_status_id'] = $pendingStatus !== '' ? $pendingStatus : 1;
		$data['pix_paid_status_id'] = $paidStatus !== '' ? $paidStatus : 2;
		$data['pix_canceled_status_id'] = $canceledStatus !== '' ? $canceledStatus : 7;

		$this->load->model('localisation/order_status');
		$data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();

		// geo zone related values
		$data['pix_geo_zone_id'] = $this->config->get('payment_pix_geo_zone_id');
		$this->load->model('localisation/geo_zone');
		$data['geo_zones'] = $this->model_localisation_geo_zone->getGeoZones();

		$data['pix_sort_order'] = $this->config->get('payment_pix_sort_order');

		// callback info related values
		$data['pix_url_callback'] = $this->config->get('payment_pix_url_callback');
		$data['pix_anti_phishing_key'] = $this->config->get('payment_pix_anti_phishing_key');
		...
		$this->response->setOutput($this->load->view('extension/ifthenpay/payment/pix', $data));
	}
	...
	{
		$this->load->language('extension/ifthenpay/payment/pix');
		if (!$this->user->hasPermission('modify', 'extension/ifthenpay/payment/pix')) {
			$this->json['error'] = $this->language->get('error_permission');
		}

		$this->load->model('setting/setting');
		$savedSettings = $this->model_setting_setting->getSetting('payment_pix');

		$savedBackofficeKey = $this->config->get('payment_pix_backoffice_key');
		if (empty($savedBackofficeKey)) {
			// if backoffice key is not saved, validate it and get accounts from ifthenpay

			$inputtedBackofficeKey = $this->request->post['payment_pix_backoffice_key'] ?? '';
			...
			if ($this->validateBackofficeKey($inputtedBackofficeKey)) {
				...
				if ($accounts !== []) {
					$savedSettings['payment_pix_accounts'] = json_encode($accounts);
				}
			}
			...
			if (!isset($this->json['error'])) {
				...
				$this->model_setting_setting->editSetting('payment_pix', $mergedConfiguration);
		...
		if (!isset($this->json['error'])) {
			...
			$this->json['redirect'] = $this->url->link('extension/ifthenpay/payment/pix', 'user_token=' . $this->session->data['user_token'], true);
		}
	...
	{
		$activateCallback = $this->request->post['payment_pix_activate_callback'];
		$savedActivateCallback = $this->config->get('payment_pix_activate_callback');

		$backofficeKey = $this->config->get('payment_pix_backoffice_key');
		$key = $this->request->post['payment_pix_key'];

		$savedKey = $this->config->get('payment_pix_key');
		...
		if (
			...
			try {
				...
				$urlCallback = $this->url->link('extension/ifthenpay/payment/pix|callback', '', true) . Gateway::PIX_CALLBACK_STRING;
				...
				$this->request->post['payment_pix_activate_callback'] = '0';
			...
			if (!isset($this->json['error'])) {
				...
				$this->request->post['payment_pix_anti_phishing_key'] = $antiPhishingKey;
				$this->request->post['payment_pix_url_callback'] = $urlCallback;
			}
		}
	}
	...
	private function validate($formData): bool
	{
		...
		if ($formData['payment_pix_key'] == '') {
			$this->json['error'] = $this->language->get('error_key_empty');
			return false;
		}

		if ($formData['payment_pix_min_value'] !== '' && (!is_numeric($formData['payment_pix_min_value']) || $formData['payment_pix_min_value'] < 0)) {
			$this->json['error'] = $this->language->get('error_min_value_format');
			return false;
		}

		if ($formData['payment_pix_max_value'] !== '' && (!is_numeric($formData['payment_pix_max_value']) || $formData['payment_pix_max_value'] < 0)) {
			$this->json['error'] = $this->language->get('error_max_value_format');
			return false;
		}

		if ($formData['payment_pix_min_value'] !== '' && $formData['payment_pix_max_value'] !== '' && $formData['payment_pix_min_value'] > $formData['payment_pix_max_value']) {
	...
	public function install(): void
	{
		if ($this->user->hasPermission('modify', 'extension/payment')) {
			...
			$this->model_extension_ifthenpay_payment_pix->install();
		}
	}
	...
	public function uninstall(): void
	{
		if ($this->user->hasPermission('modify', 'extension/payment')) {
			...
			$this->model_extension_ifthenpay_payment_pix->uninstall();
		}
	}
	...
	public function ajaxTestCallback(): void
	{
		...
		if (!isset($json)) {
			...
			$callbackUrl = $this->config->get('payment_pix_url_callback');
			$antiPhishingKey = $this->config->get('payment_pix_anti_phishing_key');
	...
	/**
	 * clear the configuration of the extension, deleting all the settings from database related to pix
	 * @return void
	 */
	public function ajaxClearConfiguration(): void
	{
		...
		$this->model_setting_setting->deleteSetting('payment_pix');
	...
	/**
	 * ajax request to request a PIX account by sending an email to Ifthenpay
	 * @return void
	 */
	public function ajaxRequestAccount(): void
	{
		...
		$data['refresh_accounts_url'] = $this->url->link('extension/ifthenpay/payment/pix.refreshAccountsCtrl', 'user_token=' . $this->session->data['user_token']);
	...
	public function ajaxRefreshAccounts(): void
	{
		...
		if ($accounts === []) {
			$json['error'] = 'No PIX accounts were found for this backoffice key';
		}
		...
		if (!isset($json)) {
			$this->model_setting_setting->editSetting('payment_pix', $savedSettings);
	...
	public function refreshAccountsCtrl()
	{
		...
		if ($accounts === []) {
			http_response_code(400);
			die('No PIX accounts were found for this backoffice key');
		}
