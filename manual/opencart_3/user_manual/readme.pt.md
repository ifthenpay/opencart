...
| ------------------------------------- | --------------------------------- |
| Link para descarregar instalador .ocmod.zip | [ifthenpay v1.4.3](https://github.com/ifthenpay/opencart/releases/download/1.4.3/ifthenpay.ocmod.zip) |
...
**MB WAY** é a primeira solução inter-bancos que permite a compra e transferência imediata por via de smartphone e tablet. Esta extensão permite gerar um pedido de pagamento ao smartphone do consumidor, e este pode autorizar o pagamento da sua encomenda na aplicação MB WAY. Este plugin usa a ifthenpay, uma das várias gateways disponíveis em Portugal.

**Payshop** é um método de pagamento que permite ao consumidor pagar com referência Payshop. Esta extensão permite gerar uma referência de pagamento que o consumidor pode usar para pagar a sua encomenda num agente Payshop ou CTT. Este plugin usa a ifthenpay, uma das várias gateways disponíveis em Portugal.
...
**Cofidis Pay** é uma solução de pagamento que facilita o pagamento de compras ao dividir o valor até 12 prestações sem juros. Este extensão usa a ifthenpay, uma das várias gateways disponíveis em Portugal.
...
| ------------------------- | ------------------------------ | ------------------------------ |
| ifthenpay v1.0.0 - v1.4.3 | Compatível | Não compatível |

</br>

## Instalação

Descarregue o ficheiro de instalação da extensão ifthenpay para Opencart 4 na página do github [ifthenpay v1.4.3](https://github.com/ifthenpay/opencart/releases/tag/v1.4.3).
...
Cada configuração de método de pagamento requer a introdução da chave de backoffice para carregar as contas disponíveis. A Chave Backoffice é dada na conclusão do contrato e é constituída por conjuntos de quatro algarismos separados por um traço (-).
...
14. **Instrução do Método de Pagamento** - (opcional) Apresenta uma mensagem de instrução no checkout. Utilizada para informar o cliente sobre o método de pagamento escolhido. Pode ser editada nos ficheiros de tradução.
...
12. **Descrição** - (opcional) Mostra esta descrição ao cliente na aplicação MB WAY. Use a variável {{order_id}} para incluir o número da encomenda.
13. **Exibir Logo do Método de Pagamento** - (opcional) Exibe o logo deste método de pagamento no checkout, desabilitar esta opção exibirá o Título do método de pagamento.
14. **Título** - Título que é exibido ao consumidor no checkout no caso de o logo não ser exibido;
15. **Instrução do Método de Pagamento** - (opcional) Apresenta uma mensagem de instrução no checkout. Utilizada para informar o cliente sobre o método de pagamento escolhido. Pode ser editada nos ficheiros de tradução.
16. **Ordenação** - (opcional) Ordena os métodos de pagamento na página de checkout de forma ascendente. Número mais baixo toma o primeiro lugar.
...
13. **Instrução do Método de Pagamento** - (opcional) Apresenta uma mensagem de instrução no checkout. Utilizada para informar o cliente sobre o método de pagamento escolhido. Pode ser editada nos ficheiros de tradução.
...
15. **Título** - Título que é exibido ao consumidor no checkout no caso de o logo não ser exibido;
16. **Instrução do Método de Pagamento** - (opcional) Apresenta uma mensagem de instrução no checkout. Utilizada para informar o cliente sobre o método de pagamento escolhido. Pode ser editada nos ficheiros de tradução.
17. **Ordenação** - (opcional) Ordena os métodos de pagamento na página de checkout de forma ascendente. Número mais baixo toma o primeiro lugar.

Clique em salvar (18) para guardar as alterações.
...
18. **Instrução do Método de Pagamento** - (opcional) Apresenta uma mensagem de instrução no checkout. Utilizada para informar o cliente sobre o método de pagamento escolhido. Pode ser editada nos ficheiros de tradução.
...
Clique em salvar (20) para guardar as alterações.
...
### Mensagem de instruções de pagamento
