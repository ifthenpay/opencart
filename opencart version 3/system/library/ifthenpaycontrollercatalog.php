...
class IfthenpayControllerCatalog extends Controller
{
    ...
    public function confirm()
    {
        ...
        if ($this->session->data['payment_method']['code'] == $this->paymentMethod) {
            ...
            } catch (Checkoutformvalexception $th) {

                $this->{$this->dynamicModelName}->log([
                    'orderInfo' => $order_info,
                    'errorMessage' => $th->getMessage()
                ], 'Error Processing Payment, passed frontend form validation but failed backend form validation.');
        ...
        try {
            if (
                ...
                if ($order_info['payment_code'] == $this->paymentMethod) {
                    ...
                    // Debug log uncomment to view data passed to success page
    ...
    {
        $output .= $this->load->view('extension/payment/ifthenpay_cofidis_info');
    }
