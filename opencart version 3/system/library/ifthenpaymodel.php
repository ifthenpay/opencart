...
class IfthenpayModel extends Model {
	...
	{
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order WHERE `payment_code` = '" . $this->paymentMethod . "' AND `order_status_id` =" . 
			$this->config->get(
    ...
    {
        return $this->db->query("SELECT * FROM " . DB_PREFIX . "ifthenpay_". $this->paymentMethod . 
	...
	{
		$this->db->query("DELETE FROM " . DB_PREFIX . "ifthenpay_". $this->paymentMethod . 
            " WHERE order_id = '" . $this->db->escape($orderId) . "'"
        );  
	}
	...
	public function deleteSettingByKey(string $key, $store_id = 0) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE store_id = '" . (int)$store_id . "' AND `key` = '" . 
            $this->db->escape($key) . "'"
        );
	}
	...
	public function getMethod($address, $total) 
    {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int) $this->config->get('payment_' . 
        ...
        if ($this->paymentStatus) {
			...
			if ($this->config->get('payment_' . $this->paymentMethod . '_showPaymentMethodLogo') && $this->request->get['route'] !== 'api/payment/methods') {
				$method_data['title'] = $gateway->getPaymentLogo(
					$this->paymentMethod, 
					$this->config->get('config_secure') ? rtrim(HTTP_SERVER, '/') : rtrim(HTTPS_SERVER, '/')
				);
			} else {
	...
	{
		return $this->db->query("SELECT * FROM " . DB_PREFIX . "order_history WHERE order_id = '" . 
