...
class IfthenpaygatewayConfigForm extends ConfigForm
{
	...
	public function generateIfthenpaygatewayPaymentMethodsHtml(array $paymentMethodGroupArray, array $gatewayKeySettings, array $storedMethods = []): string
	{
		...
		foreach ($paymentMethodGroupArray as $paymentMethodGroup) {
			...
			foreach ($paymentMethodGroup['accounts'] as $account) {
				...
				if ($index === 0) {
					$hiddenInput = <<<HTML
					<input type="hidden" name="payment_ifthenpaygateway_methods[{$paymentMethodGroup['Entity']}][account]" value="{$account['Conta']}">
					HTML;
				}
				$index++;
			...
			if ($accountOptions !== '') {
				...
				<select {$selectDisabledStr} name="payment_ifthenpaygateway_methods[{$paymentMethodGroup['Entity']}][account]" id="{$paymentMethodGroup['Entity']}" class="form-control">
					{$accountOptions}
				</select>
				HTML;
	...
	public function generateSelectedDefaultHtml(array $paymentMethodGroupArray, array $storedMethods = [], string $storedDefaultPaymentMethod): string
	{
		...
		$html = <<<HTML
		<select name="payment_ifthenpaygateway_default_method" id="payment_ifthenpaygateway_default" class="form-control">
			{$accountOptions}
		</select>
		HTML;
