...
abstract class ConfigForm
{
    ...
    protected function setGatewayBuilderData(): void
    {
        ...
        if (isset($this->ifthenpayController->request->post['payment_' . $this->paymentMethod . '_show_instruction'])) {
            $this->data['payment_' . $this->paymentMethod . '_show_instruction'] = $this->ifthenpayController
                ->request->post['payment_' . $this->paymentMethod . '_show_instruction'];
        } else {
            $this->data['payment_' . $this->paymentMethod . '_show_instruction'] = $this->ifthenpayController
                ->config->get('payment_' . $this->paymentMethod . '_show_instruction');
        }
    ...
    private function validateCofidisMinMax(): bool
    {
        if (
            ...
            if ((isset($limits['min']) && isset($limits['max']))) {
                ...
                if ($max > $limits['max']) {
                    $this->ifthenpayController->error['warning'] = $this->ifthenpayController->language->get('error_invalid_max_number_larger_than_ifthenpay');
                }
    ...
    private function validateIfthenpaygateway(): bool
    {
        ...
        // paymentMethods
        $paymentMethods = $this->ifthenpayController->request->post['payment_' . $this->paymentMethod . '_methods'] ?? [];
    ...
    private function generateAndSavePaymentLogo($paymentMethodGroups, $paymentLogoType): bool
    {
        ...
        if (
            !file_exists(DIR_CATALOG . 'view/theme/default/image/ifthenpay/ifthenpaygateway_default.png') ||
            !file_exists(DIR_CATALOG . 'view/theme/default/image/ifthenpay/ifthenpaygateway.png') ||
            !is_writable(DIR_CATALOG . 'view/theme/default/image/ifthenpay/ifthenpaygateway_default.png') ||
            !is_writable(DIR_CATALOG . 'view/theme/default/image/ifthenpay/ifthenpaygateway.png')
        ) {
        ...
        if ($paymentLogoType != 'composite' || count($paymentLogos) <= 0) {
            ...
            copy(DIR_CATALOG . 'view/theme/default/image/ifthenpay/ifthenpaygateway_default.png', DIR_CATALOG . 'view/theme/default/image/ifthenpay/ifthenpaygateway.png');
            return true;
        }
        ...
        imagepng($combinedImage, DIR_CATALOG . 'view/theme/default/image/ifthenpay/ifthenpaygateway.png');
    ...
    public function processForm(): void
    {
        ...
        if ($this->paymentMethod == 'cofidis' && !$this->validateCofidisMinMax()) {
            return;
        }
