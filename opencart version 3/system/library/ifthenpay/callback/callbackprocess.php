...
class CallbackProcess
{
	...
	protected function executePaymentMethodNotFound(): void
	{
		if (isset($this->request['test']) && $this->request['test'] === 'true') {
			...
			$response = [
				'status' => 'warning',
				'message' => 'Payment Method not found for payment method ' . $this->paymentMethod
			];
			die(json_encode($response));
		}

		http_response_code(404);
		die('Método de Pagamento não encontrado');
	}
	...
	protected function changeIfthenpayPaymentStatus(string $status): void
	{
		switch ($this->request['payment']) {
			...
			case Gateway::COFIDIS:
				...
				$this->ifthenpayController->model_extension_payment_cofidis->updatePaymentStatus(
					$this->paymentData['id_ifthenpay_cofidis'],
					$status
				);
				break;
			default:
