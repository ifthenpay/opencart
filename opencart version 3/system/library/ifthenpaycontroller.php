...
class IfthenpayController extends Controller
{
    ...
    public function insertResendPaymentDataButton(&$route, &$data, &$output)
    {
        ...
        if (!empty($ifthenpayPayment) && $ifthenpayPayment['status'] === 'pending') {
            ...
            if ($this->paymentMethod === Gateway::MBWAY) {
                ...
                $variablesForJavascript['mbwaySvgUrl'] = 'admin/view/image/payment/ifthenpay/mbway.png';
    ...
    public function testCallback()
    {
        try {
            ...
            $orderId = isset($this->request->post['order_id']) ? $this->request->post['order_id'] : '';
            ...
            // fill callback parameters for given method
            switch ($method) {
                case 'multibanco':
                    ...
                    break;
                case 'mbway':
                    $ifthenpayPayment = $this->{$this->dynamicModelName}->getPaymentByOrderId($orderId)->row;
                    $callbackUrl = str_replace('[REQUEST_ID]', $ifthenpayPayment['id_transacao'] ?? '', $callbackUrl);
                    break;
                case 'payshop':
